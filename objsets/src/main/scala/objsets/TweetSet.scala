...
/**
  ...
  * search
  * tree. Every branch in the tree has two children (two `TweetSet`s). There
  * is an
  * invariant which always holds: for every branch `b`, all elements in the left
  * subtree are smaller than the tweet at `b`. The elements in the right
  * subtree are
  * larger.
  *
  * Note that the above structure requires us to be able to compare two
  * tweets (we
  * need to be able to say which of two tweets is larger, or if they are
  * equal). In
  * this implementation, the equality / order of tweets is based on the
  * tweet's text
  * (see `def incl`). Hence, a `TweetSet` could not contain two tweets with
  * the same
  ...
  * elements
  * of the set can be found quickly. If you want to learn more you can take a
  * look
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    lazy val acc2: TweetSet = {
      if (p(elem)) acc.incl(elem)
      else acc
    }
    left.filterAcc(p, acc2)
    right.filterAcc(p, acc2)
  }

  def union(that: TweetSet): TweetSet = {
    lazy val newSet = {
      if (!that.contains(elem)) that.incl(elem)
      else that
    }
    left union newSet
    right union newSet
